...
class BackendDevice:
    ...
    def __getattr__(self, name):
        if self.quant and "device_name" not in name and "tile_size" not in name:
...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        return NDArray.make(
            new_shape, device=self.device, handle=self._handle
        )
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        # Ensure the new shape has at least as many dimensions as the original shape
        # if len(new_shape) < len(self._shape):
        #     raise ValueError("New shape must have at least as many dimensions as the original shape")


        new_strides = []
        original_shape_reversed = list(self._shape[::-1])
        new_shape_reversed = list(new_shape[::-1])

        for i in range(len(new_shape_reversed)):
            # If the shape is exhausted, it's implied that it's of size 1 for the rest of the dimensions
            current_dim = original_shape_reversed[i] if i < len(original_shape_reversed) else 1

            if current_dim == new_shape_reversed[i] and i < len(self._strides[::-1]):
                new_strides.append(self._strides[::-1][i])
            elif current_dim == 1:
                new_strides.append(0)
            elif i >= len(self._strides[::-1]):
                new_strides.append(0)
            else:
                #import pdb; pdb.set_trace()
                raise ValueError(f"Dimension mismatch: cannot broadcast {current_dim} to {new_shape_reversed[i]}")
    ...
    def __getitem__(self, idxs):
        ...
        for dim, sl in enumerate(idxs):
            dim_len = (sl.stop - sl.start + sl.step - 1) // sl.step
            new_shape.append(dim_len)
            new_strides.append(self._strides[dim] * sl.step)
            new_offset += self._strides[dim] * sl.start
    ...
    def flip(self, axes):
        ...
        for axis in axes:
            # Adjust the stride for the given axis
            new_strides[axis] = -self._strides[axis]
            # Adjust the offset for the given axis
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        new_shape = tuple(s + axes[dim][0] + axes[dim][1] for dim, s in enumerate(self.shape))
        padded_array = NDArray.make(new_shape, device=self.device)

        # Initialize with zeros
        padded_array.fill(0)

        # Calculate the start indices for each axis in the padded array
        start_indices = [axes[dim][0] for dim in range(self.ndim)]

        # Iterate over each element in the original array
        for idx in np.ndindex(self.shape):
            # Calculate the destination index in the padded array
            dest_idx = tuple(idx[dim] + start_indices[dim] for dim in range(self.ndim))

            # Copy the element from the original array to the padded array
            padded_array[dest_idx] = self[idx]
