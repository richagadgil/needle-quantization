...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    // Calculate multi-dimensional index based on gid
    for (int i = num_dims - 1; i >= 0; --i) {
      size_t index = remaining % shape.data[i];
      remaining /= shape.data[i];
      in_index += strides.data[i] * index;
    }
    out[gid] = a[in_index];
  }
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
    ...
    if (gid < size) {
        ...
        for (int i = num_dims - 1; i >= 0; --i) {
            indices[i] = remaining % shape.data[i];
            remaining /= shape.data[i];
            in_index += strides.data[i] * indices[i];
        }
        out[in_index] = a[gid];
    }
}
...
__global__ void MatmulKernel(const float* a, const float* b, float* out, uint32_t m, uint32_t n, uint32_t p) {
    ...
    for (int ph = 0; ph < (n - 1) / TILE + 1; ++ph) {
        if (row < m && ph * TILE + tx < n)
            tile_a[ty][tx] = a[row * n + ph * TILE + tx];
        else
            tile_a[ty][tx] = 0.0;

        if (col < p && ph * TILE+ ty < n)
            tile_b[ty][tx] = b[(ph * TILE + ty) * p + col];
        else
            tile_b[ty][tx] = 0.0;
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 dimGrid((P - 1) / TILE + 1, (M - 1) / TILE + 1);
...
__global__ void reduceMaxKernel(const float* d_a, float* d_out, size_t size, size_t reduce_size) {
    int out_idx = blockIdx.x * blockDim.x + threadIdx.x;

    if (out_idx < size) {

      float max_val = 0;  // Initialize with the first value
      for (int i = 0; i < reduce_size; i++) {
          float current_val = d_a[out_idx * reduce_size + i];
...
__global__ void reduceSumKernel(const float* d_a, float* d_out, size_t size, size_t reduce_size) {
    int out_idx = blockIdx.x * blockDim.x + threadIdx.x;
