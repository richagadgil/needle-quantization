...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  while (dim > -1) {
    ...
    gid++;
    dim = num_dims - 1;
    while (indices[dim]+1 == shape.data[dim]) {
      indices[dim] = 0;
      dim--;
    }
    if (dim > -1) {
...
void Compact(const CudaArray& a, CudaArray* out, std::vector<int32_t> shape,
  ...
  CudaDims dim = CudaOneDim(out->size);
  printf("Grid : {%d, %d, %d} blocks. Blocks : {%d, %d, %d} threads.\n", dim.grid.x, dim.grid.y, dim.grid.z, dim.block.x, dim.block.y, dim.block.z);
  printf("size %u", VecToCuda(shape).size);
...
__global__ void reduceMaxKernel(const float* d_a, float* d_out, size_t reduce_size) {
    int out_idx = blockIdx.x * blockDim.x + threadIdx.x;

    float max_val = d_a[out_idx * reduce_size];  // Initialize with the first value
...
__global__ void reduceSumKernel(const float* d_a, float* d_out, size_t reduce_size) {
    int out_idx = blockIdx.x * blockDim.x + threadIdx.x;

    float val = d_a[out_idx * reduce_size];  // Initialize with the first value
    for (int i = 1; i < reduce_size; ++i) {
        float current_val = d_a[out_idx * reduce_size + i];
        val += current_val;
    }
